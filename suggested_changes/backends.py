...
class BuildBackend(metaclass=abc.ABCMeta):
    ...
    def dependency_url(self, line: str, parent_path: str | None = None) -> str:
        """Returns the URL for the dependency as it should appear in the lock file."""
        return self.expand_line(line=line)
...
class PDMBackend(BuildBackend):
    ...
    def dependency_url(self, req: str, parent_path: str | None = None) -> str:
        if parent_path is not None and "file:///${PROJECT_ROOT}" in req:
            return req.replace("file:///${PROJECT_ROOT}", parent_path)
        return self.expand_line(req=req)
